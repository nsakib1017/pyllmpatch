"""Structural-directive generator (M4+): turn the ground-truth control-flow SKELETON
DIFF into a short, actionable natural-language instruction to inject into the LLM
repair prompt for "Different control flow" objects.

Motivation
----------
The dominant hard-regime bottleneck is "Different control flow" (~52% of divergences):
the decompiler emitted the right *statements* but the wrong *nesting* — a dropped
``if``/``for``/``while``/``try``/``with`` wrapper, or statements stranded at a flatter
(or different) level than the ground truth. An earlier heuristic pseudo-CFG
"reconstruction brief" (``reattach_source_code_object._gt_reconstruction_brief``) dumped
the WHOLE GT control skeleton into the prompt and A/B-tested as a clean NULL — a generic
dump is not actionable.

This generator is the untried lever: instead of a dump, it emits a **targeted delta** —
it tells the model EXACTLY which construct the ground truth has that the decompiled
source is missing/mis-nesting, naming (a) the construct type, (b) its header/condition
recovered VERBATIM from GT bytecode, and (c) which statements belong inside it (by their
salient content, since the model already has the derived source text).

How the skeleton diff is computed
---------------------------------
Two complementary, fully deterministic extractors, unified into "construct records"
keyed by a precise per-construct SIGNATURE, then diffed GT-minus-derived:

  1. **bc_to_cft skeleton** (PyLingual's DETERMINISTIC control-flow reconstructor —
     no model, no GPU; the linchpin per the project memory). When the object reduces to a
     clean template tree we walk it and read ``if``/``for``/``while``/``with`` constructs
     straight from the structured members (precise bodies + correct nesting).
  2. **bytecode-window fallback.** Real GT/derived objects are frequently *irreducible*
     (``bc_to_cft`` returns the ``CDG`` fallback), so we ALSO recover constructs directly
     from the local bytecode window: a forward conditional branch whose header condition
     is cleanly recoverable (``if``/ternary), a ``FOR_ITER`` (``for``), and the exception
     table (``try``). This makes the signal robust on exactly the hard real cases where
     the tree cannot structure.

A construct is emitted as a directive only when it is present in the GT record set and
ABSENT from the derived record set for the same kind+signature — i.e. genuinely dropped
or mis-nested. Signatures are the bytecode-recovered header text (a condition, a
``for <target> in <iter>``, a protected call / exception type), so a construct present on
both sides (even if a leaf inside it differs) CANCELS and produces no directive. This is
the precision guarantee: a leaf-only change, or a shared construct, never fires.

Scope / precision-over-coverage
-------------------------------
We only emit a construct we can describe CONFIDENTLY from GT bytecode: an ``if`` whose
condition is a simple comparison / identity / membership / truthiness (calls, boolean
ops, etc. DEFER), a ``for`` (target always; iterable when it is a simple name/attr), a
``try`` from the exception table, a ``with``/``while`` from the structured tree. Anything
we cannot name confidently is suppressed rather than emitted vaguely. If no clean
structural difference remains, the public function returns ``None`` (the caller then
injects no directive). It never raises.
"""
...
def _jump_target(inst) -> int | None:
    """The absolute target offset of a jump, from ``argval`` or the ``to N`` argrepr."""
...
def _stmt_groups(insts: list) -> list[list]:
    """Split instructions into statement groups by ``starts_line`` (skipping noise).
    A group begins at the first instruction carrying a line number; continuation
    instructions (``starts_line`` None) attach to it."""
    groups: list[list] = []
...
def _simple_value(insts: list) -> str | None:
    """Render a run of instructions that computes ONE value from names / attribute
    chains / simple constants, or None for anything richer (calls, arithmetic, etc.)."""
    stack: list[str] = []
    for i in _nonskip(insts):
        op = i.opname
        if op == "GET_ITER":
...
def _leading_callee(insts: list) -> str | None:
    """The dotted callee at the START of an instruction run (``data.read`` from
    ``LOAD data; LOAD_ATTR read; CALL ...``), stopping at the first argument load / CALL."""
    parts: list[str] = []
    for i in _nonskip(insts):
        op = i.opname
        if not parts:
...
def _loop_body_first_stmt(body_insts: list) -> str | None:
    """Describe the first REAL statement of a loop body, skipping the leading loop-target
    binding (``UNPACK_SEQUENCE`` / ``STORE_<target>``) which is not a source statement."""
    stripped: list = []
    skipping = True
    for i in body_insts:
...
def _describe_statement(group: list) -> str | None:
    """A short human phrase for a single statement group: an assignment
    (``x = data.read(...)`` / ``total += n``), a ``return ...``, or a bare call
    (``io.BytesIO(...)``). None when it cannot be rendered simply."""
...
def _for_target(full: list, f_idx: int) -> str | None:
    """The loop target(s) stored right after ``FOR_ITER`` (handles tuple unpacking)."""
    names: list[str] = []
...
def _for_iterable(full: list, f_idx: int) -> str | None:
    """The iterable expression feeding ``FOR_ITER`` — recovered from the statement group
    ending in the preceding ``GET_ITER`` (simple name/attr chains only)."""
    gi = None
...
def _window_if_records(full: list) -> list[dict]:
    """Every FORWARD conditional branch whose header condition is cleanly recoverable
    from GT bytecode becomes an ``if`` record (this also catches ternaries)."""
    records: list[dict] = []
...
def _handler_exc_type(insts: list, target: int) -> str | None:
    """The (possibly dotted) exception type of a handler at ``target``, recovered from the
    ``LOAD_GLOBAL/NAME (+ LOAD_ATTR chain); CHECK_EXC_MATCH`` pattern — independent of
    whether the handler body is trivial (``structural._handler_skeleton`` only reports the
    type for bare ``pass`` handlers, so we recover it ourselves for typed handlers with a
    real body). None for a bare ``except:`` or when the type is not a simple name chain."""
    window: list = []
    started = False
    for i in insts:
...
def _is_with_handler(insts: list, target: int) -> bool:
    """A ``with``-statement's exception cleanup also lives in the exception table; its
    handler starts a ``WITH_EXCEPT_START`` cleanup. Detect it so we never mislabel a
    ``with`` as a ``try``."""
    started = False
    for i in insts:
        if getattr(i, "offset", -1) == target:
...
def _classify(node):
    """Classify a structured template node by its member keys (version-robust).
    Returns ``(kind, header_node, body_node)`` or None."""
...
def _walk_tree(node, out: list, depth: int = 0):
    """Collect ``(kind, header_node, body_node, node)`` for every structured construct."""
...
def _constructs(bc) -> list[dict]:
    """All construct records for a code object, from the bc_to_cft tree (primary) plus
    the bytecode-window / exception-table fallbacks, deduped by (kind, sig) preferring the
    tree record (richer body description)."""
...
def _unmatched(gt_records: list[dict], der_records: list[dict]) -> list[dict]:
    """GT constructs with no same-(kind,sig) construct available in the derived side —
    i.e. dropped or mis-nested wrappers. Multiset-consuming so duplicates are handled."""
    pool: dict[tuple, int] = {}
...
def structural_repair_directive(gt_code_object, derived_code_object) -> str | None:
    """A short, imperative structural directive describing the control-flow construct(s)
    the ground truth has that the decompiled source is missing / mis-nesting, or None when
    no clean structural difference can be described confidently. Never raises.

    Intended for injection into the LLM repair prompt for "Different control flow" objects.
    """
    try:
