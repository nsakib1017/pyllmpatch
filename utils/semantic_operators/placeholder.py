"""Placeholder-statement operator: deterministically reconstruct a module/class-body
statement PyLingual gave up on and replaced with the fixed marker string
``'Decompiler error: line too long for translation. Please decompile this statement
manually.'``

When the deferred statement was a plain assignment of a *literal* value
(``NAME = <literal>``), the ground-truth bytecode stores that literal directly, so we
read the correct value straight from GT and re-emit the assignment. Two GT idioms are
recognized:

  * ``LOAD_CONST <literal>; STORE_NAME/STORE_FAST <name>``  ->  ``<name> = <repr(value)>``
  * ``BUILD_LIST 0; LOAD_CONST <tuple>; LIST_EXTEND 1; STORE_NAME <name>``
        ->  ``<name> = [<elems...>]``   (the list-display the constant tuple came from)

Alignment is deterministic and conservative: exactly one marker Constant must appear in
the fragment, and exactly one GT literal-store target must be absent from the derived
object's stores (that is the statement the marker stands in for). Anything else — a
call, a name load, a comprehension, a non-round-trippable constant, an annotated
assignment, or any ambiguity — defers (returns None). The loop's recompile + oracle
accept gate is the final backstop, but this operator never emits a value it did not read
verbatim from GT.
"""
...
def _gt_literal_store_targets(insts: list) -> list[tuple[str, object, str]]:
    """Every ``(name, value, kind)`` where GT builds a literal and immediately stores it
    into a bare name. ``kind`` is ``"scalar"`` (LOAD_CONST->STORE) or ``"list"`` (the
    BUILD_LIST 0 / LOAD_CONST tuple / LIST_EXTEND 1 / STORE idiom)."""
...
def _is_annotated(insts: list, name: str) -> bool:
    """True if GT stores an annotation for ``name`` (``__annotations__['name'] = ...``),
    which means the deferred statement was ``name: T = <literal>`` and re-emitting a plain
    ``name = <literal>`` would drop the annotation — so defer."""
...
def _marker_statement(fragment: str) -> ast.Expr | None:
    """The unique bare ``Expr`` statement whose value is the line-too-long marker
    Constant, or None if it does not appear exactly once as such."""
    try:
...
def line_too_long_candidate(gt_code_object, derived_code_object, fragment: str,
    ...
    """Reconstruct a ``NAME = <literal>`` statement PyLingual replaced with the
    line-too-long marker. Returns ``{"text", "operator", "opname", "kind",
    "confidence"}`` or None to defer.

    Regime guard: the loop must be in the "Different bytecode" state (any failed_offset —
    a bare-literal store that is length-equal to the marker docstring fails at offset 0,
    while the list-extend idiom differs in length and fails with offset None).
    """
...
def _ordered_store_names(insts: list) -> list[str]:
    """GT store names in bytecode order (any store opcode). Used only to find the
    statement adjacent to the dropped one, so relative source position is preserved."""
    out: list[str] = []
    for ins in insts:
        if getattr(ins, "opname", None) in _ANY_STORE_OPCODES:
            n = _store_name(ins)
...
def _unique_assign_target(tree: ast.AST, name: str) -> ast.Assign | None:
    """The single ``ast.Assign`` statement that binds ``name`` to a bare ``Name`` target,
    or None if that name is assigned zero or more-than-once anywhere in ``tree``."""
...
def dropped_assignment_candidate(gt_code_object, derived_code_object, fragment: str,
                                 repair_context: dict | None) -> dict | None:
    """Re-insert a scalar ``NAME = <literal>`` assignment PyLingual dropped entirely
    (a *pure insert* — unlike ``line_too_long_candidate`` there is no marker Constant to
    overwrite). Returns ``{"text", "operator", "opname", "kind", "confidence"}`` or None.

    The value is read verbatim from GT bytecode (``LOAD_CONST <literal>; STORE <name>``).
    Deterministic and conservative: exactly one GT literal-store target must be absent
    from the derived object's stores; compiler dunders, annotated assignments, and
    non-round-trippable constants defer; and the drop's GT store neighbour must map to a
    UNIQUE assignment in the source so the new line lands in the right place. Anything
    ambiguous returns None; the loop's recompile + oracle gate is the final backstop.
    """
    if not repair_context:
        return None
    failed = repair_context.get("pylingual_failed_result") or {}
