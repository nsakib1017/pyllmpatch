"""Dropped-simple-statement operator: deterministically reconstruct a *single* simple
statement the decompiler DROPPED ENTIRELY, whose full source is recoverable from GT
bytecode alone. Five sub-kinds are recognized, each read verbatim from GT:

  * ``del <target>``        -- a GT ``DELETE_NAME/FAST/GLOBAL/DEREF`` (``del name``),
                               ``DELETE_ATTR`` (``del obj.attr``) or ``DELETE_SUBSCR``
                               (``del obj[key]``) with no matching derived delete.
  * ``import ...`` / ``from ... import ...`` -- a GT ``IMPORT_NAME`` (+ optional
                               ``IMPORT_FROM`` / ``STORE_*``) statement absent in derived.
                               Module, level (leading dots), fromlist and bound name are
                               all read from the ``LOAD_CONST(level) LOAD_CONST(fromlist)
                               IMPORT_NAME`` prologue and the following store run.
  * ``assert <test>[, <msg>]`` -- the ``... POP_JUMP ... LOAD_ASSERTION_ERROR ... RAISE``
                               idiom, guarded by a truthiness/identity jump over a
                               bytecode-recoverable simple test (name / attr chain /
                               comparison / ``is`` / ``in``). A complex test (a call, a
                               boolean/arithmetic expression) DEFERS. A constant message is
                               reconstructed; a complex message DEFERS.
  * ``global <name>``       -- inferred from a STORE-scope flip: GT stores ``name`` via
                               ``STORE_GLOBAL`` where derived stores it via
                               ``STORE_FAST/STORE_NAME`` -- the source is missing a
                               ``global`` declaration. Reconstructed at the top of the body.

``nonlocal`` is DEFERRED (see the module note at the bottom): a ``STORE_DEREF`` vs
``STORE_FAST`` flip is indistinguishable from an ordinary closure cell-variable capture,
so it cannot be reconstructed without false positives.

Contract (identical to the sibling ``placeholder.dropped_assignment_candidate``):

    op(gt_code_object, derived_code_object, fragment, repair_context)
        -> {"text","operator","opname","kind","confidence"} | None

The operator is deterministic, conservative and NEVER raises. It emits a candidate only
when EXACTLY ONE dropped statement of ONE kind is unambiguously recoverable from GT; a
drop count of 0 or >1, an ambiguous target, or the wrong failure regime returns None. The
splice position mirrors the sibling: ``import``/``global`` go to the top of the body,
``del``/``assert`` are anchored beside the uniquely-mappable neighbouring GT store. Every
emitted fragment is re-parse-guarded. The loop's recompile + oracle accept gate is the
final backstop, but this operator only ever emits source it read verbatim from GT.
"""
...
def _name_of(ins) -> str | None:
    """The identifier an instruction names (argval preferred; argrepr fallback, with the
    3.11+ ``NULL + name`` LOAD_GLOBAL decoration stripped)."""
...
def _recover_load_chain(insts: list, end_idx: int) -> str | None:
    """A pure ``name(.attr)*`` load chain ENDING at ``end_idx`` (inclusive), or None."""
...
def _gt_dels(insts: list) -> list[tuple[str, int]]:
    """Reconstructable ``(text, marker_index)`` for every delete statement in ``insts``."""
...
def _recover_expr(slice_insts: list) -> str | None:
    """A simple name / attr-chain / const / comparison / is / in expression, or None."""
    stack: list[str] = []
...
def _recover_assert_msg(insts: list, ae_idx: int):
    """Return the message repr string, ``None`` (no message), or ``_DEFER`` (a message is
    present but not a lone round-trippable constant, so we must not drop it)."""
    between: list = []
    k = ae_idx + 1
...
def _neighbor_splice(gt: list, marker_idx: int, fragment: str, tree: ast.AST,
    ...
    """Insert ``stmt_text`` beside the GT store neighbouring ``marker_idx`` -- preferring
    the immediately preceding store (splice AFTER its unique source assignment); else the
    following store (splice BEFORE). Mirrors dropped_assignment_candidate's anchoring."""
    prec = foll = None
...
def _top_of_body_splice(fragment: str, tree: ast.AST, stmt_text: str) -> str | None:
    """Insert ``stmt_text`` at the top of the fragment's scope body, after a leading
    docstring. The scope is the single enclosing def/class if the fragment is one, else the
    module body."""
    body = tree.body
...
def dropped_statement_candidate(gt_code_object, derived_code_object, fragment: str,
    ...
    """Reconstruct a single dropped ``del`` / ``import`` / ``assert`` / ``global``
    statement from GT bytecode, or None to defer. Never raises."""
    try:
