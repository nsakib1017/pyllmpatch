...
def extract_top_level_objects_lenient(src: str) -> List[TopObj]:
    """Split source into top-level import, class, function, and global blocks."""
...
def _pack_objects_to_chunks(objs, max_tokens_per_chunk: int, enc) -> list[str]:
    """Pack top-level objects into token-bounded chunks without splitting them."""
...
def _merge_adjacent_chunks(chunks: list[str], max_tokens_per_chunk: int, enc) -> list[str]:
    """Merge adjacent chunks when the combined text still fits the token budget."""
    if not chunks:
        return chunks
    merged: list[str] = []
    carry = chunks[0]
...
def chunk_top_level_objects_lenient_merged(
    ...
    """Chunk content by top-level objects while respecting a token budget."""
