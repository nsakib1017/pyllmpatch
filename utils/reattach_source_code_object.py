...
def repair_mismatching_code_objects(
    ...
    *,
    ...
    iteration = 0
    # phase-local bindings, declared here so the phase helpers can rebind them
    acceptance_reason = None
    accepted = None
    candidate_summary = None
    current_bytecodes = None
    current_code_objects = None
    current_text = None
    ...
    source_path = None
    source_text = None
    step_before_rows = None
    step_pylingual_verification = None
    step_rows = None
    step_summary = None
    target_row = None
    target_score_after = None
    target_score_before = None
    updated_text = None

    def _repair_mismatched_targets() -> None:
        nonlocal acceptance_reason, accepted, accepted_this_iteration, candidate_summary, current_bytecodes, current_code_objects
        nonlocal current_pyc, current_pylingual_verification, current_source, current_summary, current_text, extracted_before
        nonlocal fragment_path, gt_bytecode, gt_code_object, gt_mapping_resolution, gt_row, gt_target_identity
        nonlocal next_pyc, next_source, prompt_record, qualname, repair_context, replacement_text
        nonlocal source_path, source_text, step_before_rows, step_index, step_pylingual_verification, step_rows
        nonlocal step_summary, target_row, target_score_after, target_score_before, unsupported_module_body_repair
        # rebound inside _repair_module_target; declared here so nonlocal can bind
        candidate_text = None

        def _repair_module_target() -> bool:
            """Repair the module body. True means: skip to the next target."""
            nonlocal acceptance_reason, accepted, accepted_this_iteration, candidate_text, current_bytecodes, current_code_objects
            nonlocal current_pyc, current_pylingual_verification, current_source, current_summary, current_text, derived_bytecode
            nonlocal derived_code_object, extracted_before, gt_bytecode, gt_code_object, next_pyc, next_source
            nonlocal repair_context, replacement_text, step_pylingual_verification, step_summary
            current_text = _load_text(current_source)
        ...
        def _evaluate_candidates() -> None:
            nonlocal candidate_text, structure_ok, structure_reason
    ...
    def _repair_extra_targets() -> None:
        nonlocal acceptance_reason, accepted, accepted_this_iteration, current_pyc, current_pylingual_verification, current_source
        nonlocal current_summary, current_text, extracted_before, fragment_path, next_pyc, next_source
        nonlocal qualname, replacement_text, step_before_rows, step_index, step_pylingual_verification, step_rows
        nonlocal step_summary, target_row, target_score_after, target_score_before, updated_text
    ...
    def _repair_missing_targets() -> None:
        nonlocal acceptance_reason, accepted, accepted_this_iteration, current_bytecodes, current_code_objects, current_pyc
        nonlocal current_pylingual_verification, current_source, current_summary, current_text, fragment_path, gt_bytecode
        nonlocal gt_code_object, gt_mapping_resolution, gt_row, gt_target_identity, next_pyc, next_source
        nonlocal prompt_record, qualname, repair_context, replacement_text, source_path, source_text
        nonlocal step_before_rows, step_index, step_pylingual_verification, step_rows, step_summary, target_score_after
        nonlocal target_score_before, updated_text
