"""Miner-4 SWEEP operators: repair every instance of a pattern in ONE pass.

Decompiled malware carries a median of ~4 orphan `try:` and ~5 dangling handlers per file,
and the parser only ever reports the first one, so one-error-at-a-time operators cannot
converge. These sweeps rewrite all occurrences at once.
"""
import re
...
def _nb(lines, j, step, stop=None):
    """next non-blank index from j inclusive walking by step; None if none."""
    while 0 <= j < len(lines):
        if lines[j].strip():
            return j
        j += step
...
def _suite_end(lines, i, indent):
    """index one past the last line of the suite whose header is lines[i] at `indent`."""
    end = len(lines)
...
def sweep_fold_postinserted_handler_body(source, error=None):
    """Re-indent handler statements PyLingual left outside their `except:` clause.

        except:
            pass  # postinserted
        hostname_pc = 'None'      <- really the handler body, emitted at the OUTER indent
        else:  # inserted

    Fires only on that exact signature (handler suite is the decompiler's own
    `pass  # postinserted` marker, and the run at the outer indent is terminated by the
    decompiler's own `else:  # inserted` marker). Leading whitespace only: no line is added,
...
def sweep_align_handlers(source, error=None):
    """Re-indent every mis-indented `except:`/`finally:` onto the `try:` it belongs to.

    Global version of `align_dangling_handler`: for each handler-less `try:` at indent T, if the
    next handler below it sits at a DIFFERENT indent and no other `try:` intervenes, shift that
    handler clause (header + suite) by T - its indent. Leading whitespace only."""
    try:
        lines = source.splitlines(keepends=True)
        out = list(lines)
        changed = False
