"""MINER 2 -- deterministic repair operators for decompiled-malware syntactic damage.

Contract: op(source, error) -> new_source | None.  None means "declines".
"""
import ast, re
...
def _scan_line(text):
    """Scan ONE physical line -> (bracket stack, still-open quote or None)."""
...
def _split_top_level_assign(text):
    """(lhs, rest_from_first_top_level_'=') or None."""
...
def _scan_from(text, stack, q):
    """Scan one physical line CONTINUING from (stack, q)."""
    stack = list(stack)
    i, n = 0, len(text)
    while i < n:
        ch = text[i]
...
def _looks_truncated(body, nxt):
    """A line left open at EOL is decompiler TRUNCATION rather than a legitimate
    continuation.  Both guards were derived from false positives measured on the
    577-file already-valid corpus; both are about the NEXT line, not this one:
      (a) a real continuation's next line is indented DEEPER (`embed = {` then
          `    'content': '',`);
      (b) a real continuation's next line may instead begin with a closer or a comma
          (`self.emit((a, 'b')` then `)`).
    A truncated line is followed by a fresh statement at the SAME OR SHALLOWER indent.
    (An earlier version tested this line's last character instead; that rejected every
    truncation that happened to stop right after a `,` or `:` -- most of them -- and
    cost 18 of the 19 files this operator set repairs.)
    """
    if nxt is None:
        return True
    head = nxt.lstrip()[:1]
...
def line_starts(src):
    """[bool] -- does physical line i begin a new logical line?"""
...
def close_unclosed_lines(src, err=None):
    """Terminate every physical line that ends mid-string or mid-bracket.

    PyLingual emits one logical line per physical line: measured on 577 already-valid
    PyLingual malware files, only 45 of 199,442 lines (0.02%) end mid-statement, and
    those are triple-quoted SQL blocks.  A line left open at EOL is therefore decompiler
    TRUNCATION, guarded by the two continuation tests in `_looks_truncated`.
    Appends the missing quote + closers in stack order.  APPEND-ONLY.
    """
...
def invalid_lhs_rename(src, err=None):
    """`<garbage> = expr` -> `_dc_N = expr` when <garbage> is not a legal target.

    PyLingual leaks a co_consts value (MAC address, Windows path, prose sentence, an
    int, `self.2`) into the binding-name slot.  Deletion throws away the whole statement
    including the RHS call chain; renaming keeps it.  RENAME ONLY.
    """
    lines = src.splitlines(keepends=True)
...
def orphan_clause_header(src, err=None):
    """A dangling `except:`/`finally:`/`else:`/`elif` whose HEADER was never emitted.

    Insert the missing header (`try:` for except/finally, `if True:` for else/elif) at
    the clause's indent, above the contiguous run of preceding lines at indent >= I, and
    indent that run by 4 when its own minimum indent equals I.
    SYNTHESIZES one header line -> preserves_all_code = False.
    """
    lines = src.splitlines(keepends=True)
    edits = []
    for i, line in enumerate(lines):
...
# ==========================================================================
def _code_only(body):
    """Blank out string contents and strip the trailing comment, so that structural
    tests ('does this line end with a block-opening colon?') cannot be fooled by a
    `#` or `:` that lives inside a literal -- e.g. `w.setStyleSheet('color: #808080')`,
    which cost 19 false positives before this guard existed."""
...
def normalize_indent(src, err=None):
    """Snap every line whose indent is on no legal level to the nearest enclosing legal
    level.  WHITESPACE ONLY -- no token added or removed.  Fixes 'unexpected indent' and
...
def retype_mismatched_closer(src, err):
    """`{'a': [1, 2)` -> `{'a': [1, 2]`.  Rewrites the WRONG closing bracket to the one
    the open stack expects instead of deleting the expression.  Every operand kept."""
...
def drop_surplus_closer(src, err):
    """`x = d['k'])` -> `x = d['k']`.  Removes one UNMATCHED closing bracket."""
...
def fill_empty_dict_slot(src, err):
    """`{'a': , 'b': 1}` -> `{'a': None, 'b': 1}`.  PyLingual drops the value when the
    constant behind it was never recovered.  ADDS a placeholder -> not faithful."""
...
def decorator_call_rejoin(src, err=None):
    """`@Embed` + `embed = f'..', title=1)` -> `embed = Embed(f'..', title=1)`.
    PyLingual reconstructs a broken CALL as a decorator plus a headless arg list."""
    lines = src.splitlines(keepends=True)
...
def strip_juxtaposed_number(src, err):
    """`if a 0 <= b:` -> `if a <= b:`; `ge = ge 6 | 1` -> `ge = ge | 1`.
    A co_consts entry emitted where no value belongs.  DELETES the stray token."""
...
def close_orphan_try(src, err=None):
    """Every `try:` whose except/finally was never emitted gets `except Exception: pass`
    appended at the end of its suite -- ALL of them in one pass, innermost last.

    Delete-only's only recourse for an orphan try is to delete the try AND its entire
    suite (a median of 243 lines across this corpus).  SYNTHESIZES 2 scaffold lines.
    """
    lines = src.splitlines(keepends=True)
    inserts = []
    for i, line in enumerate(lines):
...
def retype_try_else_to_except(src, err=None):
    """`try:` <suite> `else:` with no `except` between -> retype that `else:` to
    `except Exception:`.

    PyLingual renders a `try/except` as `try/else` when it mis-labels the handler edge.
    Retyping the clause KEEPS ITS ENTIRE BODY, where the alternative (delete-only) drops
    both the handler body and everything the try guarded.  One keyword rewritten.
    """
    lines = src.splitlines(keepends=True)
    out, changed = list(lines), False
    for i, line in enumerate(lines):
...
def append_missing_colon(src, err=None):
    """A compound-statement header emitted without its trailing `:` gets one appended.

    Shows up after a truncated condition is closed: `if a.lower()!= (b + c)` with no
    colon.  APPEND-ONLY -- one character, nothing removed.
    """
    lines = src.splitlines(keepends=True)
    starts = line_starts(src)
    out, changed = list(lines), False
...
def pipeline(src, globals_=None, locals_=None, max_rounds=200):
    """GLOBAL passes are damage-signature driven, so they run unconditionally to a
    fixpoint; the error-driven LOCAL passes then clean up what is left, and each global
    pass gets another chance after every local fix."""
