"""Miner-4 candidate deterministic operators. Signature: (source, error) -> str|None."""
...
def _comma_positions(text):
    """(index, depth_after_open_brackets) of every comma outside strings."""
...
def _close_line(text):
    """Append the closers for whatever brackets are still open (and a ':' for a block header)."""
...
def truncate_partial_element(source, error=None):
    """Close a decompiler-TRUNCATED collection at its last COMPLETE element.

    PyLingual stops emitting a constant sequence after 51 elements and cuts mid-element, so
    `balance_delimiters`' blind close produces a dangling key (`..., 'Value'}`) that still will
    not parse. This cuts back to the last comma that is directly inside the innermost still-open
    bracket, then closes every open bracket (and re-adds the `:` when the line is a block header).
    It DISCARDS the partial trailing element, so it is lossy at the sub-expression level -- but it
    saves the other N-1 elements and the whole surrounding suite from deletion."""
    try:
