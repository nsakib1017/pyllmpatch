"""Miner-3 candidate deterministic operators for decompiled-malware syntactic repair.

Every operator has signature (lines: list[str], err) -> list[str] | None
where `lines` is the source split WITHOUT newline chars and `err` is a
SyntaxErrorInfo-ish object with .lineno/.offset/.msg.  Operators return None
when they do not fire.  None of them ever raises.
"""
...
def line_context(lines):
    """Cheap cumulative scan giving, per physical line:
       in_string  -- the line starts inside a triple-quoted string
       depth0     -- bracket depth entering the line
       stmt_start -- the line begins a new logical line (real statement)
    Desyncs on badly-damaged files are harmless: every caller is compile-gated.
    """
    n = len(lines)
    in_str = [False] * n
    depth0 = [0] * n
...
def local_orphan(lines, i, kws) -> bool:
    """True when line i's clause has NO legal owner at its own indent.

    Python's rule: the clause attaches to the header at exactly the same indent
    whose suite the preceding lines belong to.  Walk up skipping deeper lines
    (they are nested suites) and blanks; the first line at indent <= I decides.
    """
    I = indent_of(lines[i])
...
def find_opener(lines, i, kws):
    """Innermost still-open block header above line i whose keyword is in kws.

    A header at indent T can own line i only if every non-blank line strictly
    between them is indented deeper than T (i.e. is still inside T's suite).
    """
    min_ind = None
    for j in range(i - 1, -1, -1):
        s = lines[j]
...
def is_block_header(line: str, kw: str) -> bool:
    """A block-opening header. Accepts both `if x:` and the one-line compound
    form `if x: return` -- the latter is still a legal owner for a later
    `else:`/`except:` clause, and treating it as 'not a header' was the single
    biggest false-positive source on ordinary Python."""
...
def align_clause_to_opener(lines, err):
    """Re-indent an else/elif/except/finally/case clause (and its suite) so it
    lines up with the innermost still-open header it can legally attach to.

    Whitespace-only: no line is added, removed, or textually altered."""
    try:
...
def _candidate_indices(lines, err):
    """Error line first, then a small window around it (the reported line for
    'expected except/finally' points at the *try*, not at the bad clause)."""
...
def orphan_else_to_if_true(lines, err):
    """An `else:` with no attachable opener becomes `if True:` in place;
    an orphan `elif C:` becomes `if C:`.  No line added or removed."""
    try:
        in_str, depth0, stmt = line_context(lines)
...
def invalid_lhs_rename(lines, err):
    """`00:e0:4c:d6:86:77 = [...]` / `7AB5C494-...-5016.False = [...]` -> a
    mangled but valid identifier.  The right-hand side (the payload) is kept
    verbatim; only the name the decompiler already lost is replaced."""
    try:
        in_str, depth0, stmt = line_context(lines)
        for i in _candidate_indices(lines, err):
...
def _paren_excess(text: str):
    """Net excess of closers over openers on a single physical line,
    ignoring string literals.  Returns None if the line cannot be scanned."""
    depth = 0
    excess = 0
    q = None
    i = 0
...
def insert_missing_open_paren(lines, err):
    """`x = (a | (b << c) >> d & (-e)) >> f` where one opening paren was lost:
    re-open the expression instead of dropping the closer, so no token is lost."""
    try:
...
def synthesize_try_for_orphan_handler(lines, err):
    """An `except:`/`finally:` with no `try:` anywhere above it: wrap the
    immediately preceding statement (and its suite) in a synthesized `try:`."""
    try:
        in_str, depth0, stmt = line_context(lines)
        for i in _candidate_indices(lines, err):
            if in_str[i] or not stmt[i]:
...
def _scan_line(text: str):
    """Return (open_bracket_stack, open_quote_or_None, index_where_quote_opened)."""
    stack = []
    q = None
    qstart = -1
    i = 0
