"""Force decompiled source to compile by closing every unfinished construct.

WHY A FOURTH DESIGN. Three attempts failed on the hardest malware files, all for the same reason:
they SEARCHED. `exhaustive_repair` explores operator sequences under a compile gate, and
`gt_literal_recovery` chased one SyntaxError at a time. Both assume a small number of defects. These
files carry a median of 8-9 truncated lines plus unclosed brackets plus missing handlers, all at once,
so the search space explodes and no single fix ever reaches a compiling state.

This module does not search. Every repair is a LOCAL, DETERMINISTIC completion of a construct the
decompiler left unfinished -- close the string, close the bracket, give the `try` a handler, give the
header a body. Each edit provably removes exactly one syntax error and introduces no new one, so the
process converges instead of exploring.

WHAT THIS IS NOT. Compiling is not the same as correct. Closing a bracket or synthesising
`except Exception: pass` INVENTS structure that was never in the malware. This module therefore keeps
a ledger of every edit and marks any touched file `faithful=False`. A caller must not report a closed
file as a faithful repair; the honest headline for these is "made analysable", not "recovered".

It never deletes a line -- that is the one thing separating this from the delete-only fallback, which
is why closure results may be counted as non-deletion repairs while still being flagged unfaithful.
"""
...
def close_unterminated_string(line):
    """Append the closing quote a truncated line is missing. Returns ``(line, changed)``."""
...
def close_line_brackets(line):
    """Close brackets opened and left open *within a single line*. ``(line, count)``.

    A truncated line must be closed on its own line, not at end of file: deferring the closer lets the
    open bracket swallow whatever is inserted later (a synthesised ``except``, the next statement),
    turning one defect into two.
    """
...
def close_open_brackets(text):
    """Append the closers needed to balance every bracket left open. ``(text, count)``."""
    stack = []
...
def _synthesise_handler(lines, lineno):
    """Give the innermost unterminated ``try:`` above ``lineno`` an ``except`` clause."""
...
def _insert_body(lines, lineno):
    """Give a block header with no body a ``pass``."""
    idx = min(max(lineno - 1, 0), len(lines) - 1)
    for i in range(idx, -1, -1):
...
def _insert_try(lines, lineno):
    """Give an orphaned ``except:``/``finally:`` the ``try:`` the decompiler dropped.

    PyLingual sometimes emits a handler whose ``try:`` never made it into the output (its own
    ``# postinserted`` marker often sits alongside). The suite that should have been guarded is
    everything between the enclosing block header and the handler, so wrap exactly that.
    """
    idx = lineno - 1
...
def _dedent_line(lines, lineno):
    """Pull a stray over-indented line back to the enclosing level."""
    idx = lineno - 1
    if not (0 <= idx < len(lines)):
        return False
...
def _rename_literal_target(lines, lineno):
    """``108 = [...]`` / ``1e:6c:.. = [...]``: a constant where a NAME belonged. Rename, keep payload."""
    idx = lineno - 1
    if not (0 <= idx < len(lines)):
        return False
...
def _strip_continuation(lines, lineno):
    """Remove a backslash that is not a real line continuation."""
    idx = lineno - 1
...
def _neutralise_line(lines, lineno, force=False):
    """LAST RESORT: turn an unparseable line into a string-literal expression.

    Every byte survives -- as data rather than as code -- so nothing is deleted and the parser is
    guaranteed to move on. This is the handler that makes termination provable, and it is also the
    most destructive to meaning: a neutralised line no longer executes. It is counted separately and
    always marks the file unfaithful.
    """
    idx = lineno - 1
    if not (0 <= idx < len(lines)):
        return False
...
def close_source(source, max_rounds=None):
    """Close every unfinished construct until the source parses, or the budget runs out.

    Returns ``(text, ledger)``. The ledger counts each kind of edit, always reports
    ``lines_deleted == 0``, and sets ``faithful`` only when nothing at all was changed.
    """
    ledger = {
        "strings_closed": 0,
        "brackets_closed": 0,
