...
def select_best_of_n_candidate(
    *,
    ...
    """Pure, compile-gated best-of-N candidate selection for the syntactic-repair path.

    Always generates the greedy candidate (temperature 0) first via `generate_greedy` and
    checks it with `compiles`. If it compiles, returns immediately -- `generate_sampled` is
    NEVER called in that case, which is what keeps N=1 (and "greedy already works") down to
    exactly one LLM call, matching production cost bounds.

    Only when the greedy candidate fails to compile AND `best_of_n > 1` does this call
    `generate_sampled` up to `best_of_n - 1` more times, checking each with `compiles` and
    accepting the first one that compiles. If none of the sampled candidates compile either
    (or `best_of_n <= 1`), the greedy candidate is returned -- mirroring today's
    single-greedy-attempt failure path byte-for-byte.

    Returns `(candidate, compiled)` where `compiled` is whatever the winning (or, on total
    failure, the greedy) candidate's `compiles(...)` call returned.
    """
